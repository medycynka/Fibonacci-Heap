...
public:
    ...
    FiboNode(const FiboNode &s)     : value(s.getValue()), prev(s.prev), next(s.next), child(s.child), parent(s.parent), degree(s.degree), marked(s.marked){};
    FiboNode(FiboNode &&s) noexcept : value(s.getValue()), prev(s.prev), next(s.next), child(s.child), parent(s.parent), degree(s.degree), marked(s.marked){};
    ~FiboNode() = default;

    inline bool hasChildren() { return child; };
    inline bool hasParent()   { return parent; };
    inline FiboNode<T> * getNextNodeForIteration();
    inline FiboNode<T> * getPrevNodeForIteration();
    inline void printNode();

    FiboNode& operator= (const T &input)          { value = input; return *this; };
    FiboNode& operator= (const FiboNode<T> &input);
    FiboNode& operator= (FiboNode<T> &&input) noexcept;
...
#endif //FIBONACCIHEAP_FIBONACCI_NODE_HPP
